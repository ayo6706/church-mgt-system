...
if($epr=='save')
	{
		...
		$a_sql= "INSERT  INTO leaders(username, password, position)VALUES('$user','$pass','$position')";
		if(mysqli_query($conn, $a_sql))
			header("location: leaders.php?epr=admin");
		else
	...
	if($epr=='delete')
		{
			$id= $_GET['id'];
			$delete="DELETE FROM leaders where id=$id";
        ...
        $pass = md5($pass);
		$a_sql="UPDATE leaders SET username='$user', password='$pass' WHERE id='$id' position='$position'";
		if(mysqli_query($conn, $a_sql))
			header("location: leaders.php?epr='admin'");
		else
			$msg='error:'.mysqli_error();
...
<head>
	...
	<?php
		...
		{
			$id=$_GET['id'];
			$row="SELECT * FROM leaders WHERE id='$id'";
			...
			<div class="form-grids row widget-shadow" 
				...
				<div class="form-body">
					<form method="post" enctype="multipart/form-data" action="leaders.php?epr=saveup">
		...
		<?php 
			...
			<div class="form-grids row widget-shadow" 
				...
				<div class="form-body">
					<form method="post" enctype="multipart/form-data" action='leaders.php?epr=save'> 
						<div class="form-group"> 
							<label for="title">
								Enter title
							</label> 
		...
		if ($result = mysqli_query($conn, $sql)){
			while ($row=mysqli_fetch_assoc($result)) {
				echo "<tr class='table-row'>
					...
					<td align='center'>
						<a class='btn btn-sm btn-danger' href='leaders.php?epr=delete&id=".$row['id']."'>delete</a>
						<a class='btn btn-sm btn-success' href='leaders.php?epr=update&id=".$row['id']."'>update</a>
					</td>
				</tr>";
