...
if($epr=='save')
	{
		...
		$a_sql= "INSERT  INTO hod(username, password, position)VALUES('$user','$pass','$position')";
		if(mysqli_query($conn, $a_sql))
			header("location: hod.php?epr=leaders");
		else
	...
	if($epr=='delete')
		{
			$id= $_GET['id'];
			$delete="DELETE FROM hod where id=$id";
		...
		$a_sql="UPDATE hod SET username='$user', password='$pass' position ='$position' WHERE id='$id'";
		if(mysqli_query($conn, $a_sql))
			header("location: hod.php?epr=leaders");
		else
			$msg='error:'.mysqli_error();
...
<head>
	...
	<?php
		...
		{
			$id=$_GET['id'];
			$row="SELECT * FROM hod WHERE id='$id'";
			...
			<div class="form-grids row widget-shadow" 
				...
				<div class="form-body">
					<form method="post" enctype="multipart/form-data" action="hod.php?epr=saveup">
		...
		<?php 
			...
			<div class="form-grids row widget-shadow" 
				...
				<div class="form-body">
					<form method="post" enctype="multipart/form-data" action='hod.php?epr=save'> 
						<div class="form-group"> 
							<label for="title">
								Enter title
							</label> 
		...
		if ($result = mysqli_query($conn, $sql)){
			while ($row=mysqli_fetch_assoc($result)) {
				echo "<tr class='table-row'>
					...
					<td align='center'>
						<a class='btn btn-sm btn-danger' href='hod.php?epr=delete&id=".$row['id']."'>delete</a>
						<a class='btn btn-sm btn-success' href='hod.php?epr=update&id=".$row['id']."'>update</a>
					</td>
				</tr>";
